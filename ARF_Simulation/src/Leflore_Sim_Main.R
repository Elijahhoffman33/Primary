...
Regions = list.files('ARF_Simulation/data/input/Leflore_Sets/Input_Sets/') %>% str_remove('.csv')
Regions = str_replace_all(Regions,'NA','1')
# Region = 'field_8248'
# Region = 'Full'
...
for(I in 100){
  ...
  for(Region in Regions[1:length(Regions)]){
    ...
    for(i in 1:nrow(run_grid)){
      ...
      input_path = list.files('ARF_Simulation/data/input/Leflore_Sets/Input_Sets',pattern=Region,full.names = T)
      training_path = list.files('ARF_Simulation/data/input/Leflore_Sets/Input_Sets',pattern=Region,full.names = T)
