...
old = c('Calcium_NH4OAc_Extractable_2M_KCl_displacement',
        'CEC_NH4OAc_pH_7.0_2M_KCl_displacement',
        'Magnesium_NH4OAc_Extractable_2M_KCl_displacement',
        'pH_11_Soil-Water_Suspension',
        'Potassium_NH4OAc_Extractable_2M_KCl_displacement',
        'Phosphorus_Mehlich3_Extractable',
        'Sodium_NH4OAc_Extractable_2M_KCl_displacement','SOC')
new = c('ca','cec','mg','ph','k','p','na','om')
setnames(plot,old,new)
...
### Compare arva cols
data_130 = '../Soil_Stratification_Sampling/data/input/Org_130_Full_input.csv' %>% fread

colnames(data_130)[6:21]
colnames(plot)

cols = colnames(plot)[9:30]
...
# x = melt(data_130 %>% select(any_of(colnames(data))) )
# x$Dataset = 'ARF'
# y = melt(data %>% select(any_of(x$variable)) )
# y$Dataset = 'USDA'
# plot = rbind(x,y)
# 
# ggplot(plot,aes(x=value,fill=Dataset,col=Dataset)) +
#   # geom_histogram(bins=50,alpha=.8,position = 'identity')+
#   # geom_density(alpha=.3) +
#   geom_boxplot(outlier.shape = NA) +  
#   facet_wrap(~variable,scales = 'free') +
#   theme_classic(base_size = 16)  + scale_fill_brewer(palette = 'Dark2') + scale_color_brewer(palette = 'Dark2')
...
cols = colnames(plot)[-c(10,16,17,18,20,22,24,25,27,30)]
data[,(cols):=NULL]
