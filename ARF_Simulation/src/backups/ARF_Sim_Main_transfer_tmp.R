...
Dirs = list.files('../Primary/ARF_Simulation/data/output/',full.names=F,pattern='Sim_')

Summary = parallel::mclapply(Dirs,mc.cores=14,function(Dir){
  print(Dir)
  files = list.files(paste0('../Primary/ARF_Simulation/data/output/',Dir,'/'),full.names=T)
...
json = fromJSON("data/input/mgmt_dict.json") %>% fromJSON 
...
gskmn <- cluster::clusGap(M %>% as.matrix, FUN = hclusCut, K.max = 30, B = 300)
plot(gskmn)
K = maxSE(f = gskmn$Tab[, "gap"], SE.f = gskmn$Tab[, "SE.sim"],
          method = "Tibs2001SEmax", SE.factor = 1)


MGMT_cl = data.table(field_id=names(x))
lapply(1:20,\(y){
  cl = hclusCut(M,y)$cluster
  col = paste0('cl_',y)
  MGMT_cl[,(col):=cl]
...
### Code Limbo ----
### Subset down based on stopping criteria, this is just for plotting
# selected = selected[seq(stop)]
exclude = tmp[From%in%selected | Against%in%selected][,From]
x = tmp[!Region%in%exclude]

Samples_Avgs = lapply(Samples,\(x) x[,.(Sim_Mean=mean(om),Sim_SD=sd(om))] ) %>%
  rbindlist
Samples_Avgs$Region = names(Samples) %>% as.integer





plot = copy(Matched_Fields) %>% merge(Samples_Avgs,by='Region')
plot = plot[plot[,!duplicated(paste(Region))]]
plot[order(Control_Clusters,Sim_Mean),Index:=1:nrow(plot)]
...
# data[,c("Avg_clay","Avg_silt","Avg_sand") :=
#        .(mean(gssurgo_claytotal_r),mean(gssurgo_silttotal_r),mean(gssurgo_sandtotal_r)),
#      # .(mean(sg_clay),mean(sg_silt),mean(sg_sand)),
#      .(mean(polaris_clay),mean(polaris_silt),mean(polaris_sand)),
#      'field_id']
# 

# x = TT.normalise.sum(data %>% as.data.frame,
#                      css.names = c("Avg_clay","Avg_silt","Avg_sand"))
# x1 = TT.points.in.classes(x,class.sys = 'USDA.TT',
#                           css.names = c("Avg_clay","Avg_silt","Avg_sand"))
### Non field averaged
